...
> **3.5a flex 收缩穿透（minmax 查不到的另一种溢出）**：flex 容器子项默认 `flex-shrink:1`，内容多时子项被压扁但**内容穿透到下层**（树节点/卡片互相叠压穿模）。grep `display:flex` / `flex-col` 容器后，顺带查子项有没有统一 `flex-shrink:0`（或 `flex:0 0 auto`）。倒模回来的布局若依赖 `flex-shrink:1` 的"挤压"观感，多半是参考项目用固定尺寸/滚动区兜住的——形态牌要把"子项收缩策略"记下来，否则复刻后第一版就穿模。

> **3.5b 图表时序轴：数据点序号 ≠ 真实时间轴**：复刻/倒模任何图表（曲线/热力图/柱状）时，先确认后端返回的是"有数据的天"（稀疏）还是"连续日期序列"。若前端按索引等距画 x 轴，稀疏数据会"跳着过日子"；正确做法是生成连续日期序列 + 真实时间轴映射 + 空档留白。形态牌对图表要记"数据密度语义"（连续 vs 稀疏），这决定轴怎么画。
