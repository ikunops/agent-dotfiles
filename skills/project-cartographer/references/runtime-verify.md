...
## 二·七、状态边界（不是时序问题，是"没造那个状态"）

二·六 讲的是**时序型中间态**（抢重渲染窗口）。还有一类常被漏掉的是**数据型状态边界**——它不需要抢窗口，只要把数据构造成那个样子，**稳态下就稳定复现**：

| 状态边界 | 怎么构造（低成本） | 交互点为什么会失效 |
|---|---|---|
| **零行 / 空结果** | 筛一个不存在的值、`WHERE 1=0`、选一张空表 | 依赖"存在行"的入口整群消失：单元格/行右键菜单（无行可右键）、行内编辑、批量操作、复制 |
| **查询出错 / 连接断** | 传非法参数、停掉连接、断网 | 出错分支下 `data=null`，被 `data &&` 门控的整块 UI 直接不挂载 |
| **门控未开（默认折叠/隐藏）** | 什么都不做，只看首次进入的样子 | 入口"存在但不可见"：默认折叠的条件栏、高级选项、二级菜单——用户根本不知道它在那 |
| **超宽 / 超长内容** | 选列最多的表、超长单元格 | 元素被撑到视口外（实测：`<td colSpan=全部列>` + `textAlign:center` → 按钮落在 x≈3505，视口只有 1700） |

**每个交互点在每个边界状态上问三句**（缺一不可）：

1. **它在 DOM 里吗？**（还是根本没渲染 / 被 `&&` 门控掉）
2. **它够得着吗？**（在视口内吗？容器能滚到它吗？）
3. **它在那个状态下还成立吗？**（动作的前提还在吗——右键菜单需要"有行"、批量删除需要"有选中"）

> **真实教训（2026-09-18，opscore 数据库管理模块 · 同一版本对照实验）**
> 只给方法论、**不给任何症状**的盲测跑完，结论是"前后端闭环总体健康……未发现『点了没反应』类渲染层 bug"，而"零行状态"被它自己放在末尾〈未覆盖〉里一笔带过（原话："数据量小、稳态点过……未做瞬态验证"）。
> 同一模块**同一版本**，一旦给出症状（"筛选后结果为空，想清除筛选但清不掉，只能刷新"），agent 立刻穷举出 4 个"清除筛选"入口并逐个验可达性，命中两条真问题（默认折叠的条件栏 + 空结果时无行可右键）。
> **症状的真正作用是"状态选择器"**——它指定了要构造哪个状态、把注意力聚到哪条链。无症状态下 agent 默认只走稳态，所以**边界状态不能等用户报，必须在 Step 4 强制穷举**。

**构造边界状态的纪律**：零行 / 出错 / 门控未开这三类**不需要真实用户场景就能造**，成本极低（改一个筛选值、传一个非法参数、什么都不做）。凡交互点测绘，先把这三类过一遍再收工；超宽态用"全项目列数最多的表"当探针。
